...
def match_title(url):
    # r = requests.get(f'http://sex8.cc/thread-11662508-1-13.html', headers=headers).text
    #
    # t = re.findall('<span\sid="thread_subject">(.*)</span>', r)
    #
    # print(t)
    # sys.exit(0)
...
def get_pic_url_list(page):
    s_url = 'http://sex8.cc/forum-159-' + str(page) + '.html'
    print('Url', s_url)
...
def write_urlfile(url):
    with open(f'./ch_sexual_street_urls.txt', 'a+') as f:
        f.write(url + "\n")
        f.close()


def download_pic(url):
    url_title = match_title(url)

    print('Downloading：', url_title[1])
    # write url
    write_urlfile(url_title[1])

    s = requests.get(url_title[1], headers=headers, stream=True)
    image_path = "./ch_sexual_street/Images/" + url_title[0] + '/' # 下载图片的保存路径
...
def write_page_file(start_page, end_page):
    ...
    with open(f'./page_street_history.txt', 'wb') as f:
        # for data in s.iter_content(128):
        f.write(content)
